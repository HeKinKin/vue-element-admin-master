  ...
  <div class="crm"  style="width: 1230px">
    <div style="padding-left: 10px;padding-top: 14px">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>编辑升级礼</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
        ...
        <div style="margin-left: 10px;">
          <el-form :inline="true" :rules="rules"  >
            ...
            <el-form-item label="活动类型："  size="mini">
              <el-select v-model="campaignType"  placeholder="请选择" size="mini">
                <el-option
            ...
            <br/>
            <el-form-item label="当前活动状态："  size="mini">
              <el-input
              v-model="campaignStatus1[0].dictionaryValue"
              :disabled="true"
              size="mini"
              clearable>
              </el-input>
            ...
            <br/>
            <el-form-item label="修改活动状态："  size="mini">
              <el-select v-model="editCampaignStatus" placeholder="请选择" size="mini">
                <el-option
            ...
            <el-form-item label="活动图片:">
              <img width="50px" height="50px" :src="'data:image/*;base64,' + realPath">
            </el-form-item>
            <br/>
        ...
        <el-tabs v-model="editableTabsValue2" type="card" closable @tab-remove="removeTab">
          ...
          >
            <el-form :inline="true"  class="demo-form-inline" >
              ...
              <el-form-item label="升级到">
                <el-select v-model="item.levelUpCode" style="width: 150px" size="mini" placeholder="请选择升级类型" clearable>
                  <el-option
                    ...
                    :value="parseInt(items.growthValueLimit)"
                  ></el-option>
                  <el-option style="width: auto" :disabled="true" :value="null">
                    <span>无</span>
                  </el-option>
              ...
              <!--<el-form-item label="升级到">-->
                <!--<el-select v-model="item.levelUpCode" style="width: 150px" size="mini" clearable>-->
                  <!--<el-option-->
                    <!--v-for="items in resultValue"-->
                    <!--:label="items.levelName"-->
                ...
                <!--<el-input-->
                  <!--placeholder="下一等级"-->
                  <!--v-model="resultValue[0].levelName"-->
                  <!--:disabled="true"-->
                  <!--size="mini"-->
                  <!--clearable>-->
                <!--</el-input>-->
              <!--</el-form-item>-->
              <!--<el-form-item label="此数字为临界成长值,请手动设置升级等级">-->
              <!--</el-form-item>-->
              <br/>
              ...
              <el-form-item  label="每人限参与次数" >
                <el-input-number v-model="item.rulesParticipation" :min="1" :max="5" label="描述文字" size="mini"></el-input-number>
              </el-form-item><br/>
            </el-form>
          </el-tab-pane>
        </el-tabs>
...
<script>
  ...
  export default {
    ...
    methods: {
      searchCampaignTai() {
        let workThis = this;
        console.log('入参为：' + workThis.campaignStatus)
        workThis.$axios({
          method: 'post',
          url: '/ipos/dictionary/select',
          data: {
            'body': {
              'key': workThis.campaignStatus
            }
          }
        }).then(function (res) {
          workThis.campaignStatus1 = res.data.body;
          console.log('查询到的活动状态为：' + JSON.stringify(workThis.campaignStatus1));
          workThis.lookEditStatus();
        })
      },
      lookEditStatus() {
        console.log('进来了！！！！！！！！！！！！！！！！！');
        console.log('当前状态为：' + this.campaignStatus1[0].dictionaryKey);
        let workThis = this;
        if (parseInt(workThis.campaignStatus1[0].dictionaryKey) === 1303) {
          console.log(11111111111111111111111111111111111111111)
          workThis.options2[0].disabled = true;
        }
      },
      change111() {
        let workThis = this;
        if (parseInt(workThis.campaignStatus1[0].dictionaryKey) === 1303) {
          console.log(workThis.campaignStatus1)
      ...
      getLevel() {
        ...
        workThis.$axios({
          method: 'post',
          url: '/ipos/campaign/getLevel',
          data: {
            'body': {
              'merchantId': workThis.crmMerchantId
            }
          }
        }).then(function (res) {
          workThis.level = res.data.body;
          // for (let k = 0; k < workThis.level.length - 1; k++) {
          //   workThis.level1.push(workThis.level[k])
          // }
        })
      },
      ...
      //   workThis.$axios({
      //     method: 'post',
      //     url: '/ipos/campaign/getNextLevel',
      //     data: {
      //       'body': {
      //         'growthValueLimit': workThis.sizeForm.activeGrowthValue,
      //         'merchantId': workThis.crmMerchantId
      //       }
      //     }
      //   }).then(function (res) {
      //     workThis.resultValue = res.data.body;
      //   })
      // },
      // getNextValue1() {
      //   let workThis = this;
      ...
      updateCampaign() {
        ...
        if (workThis.editCampaignStatus === '') {
          workThis.editCampaignStatus = workThis.campaignStatus;
        }
        // if (workThis.campaignStatus === '1304' && workThis.editCampaignStatus === '1302') {
        //   workThis.$message({
        //     type: 'warning',
        //     message: '当状态为已结束时，不能更改为未开始'
        //   });
        //   return false;
        // }
        // if (workThis.campaignStatus === '1304' && workThis.editCampaignStatus === '1303') {
        //   workThis.$message({
        //     type: 'warning',
        //     message: '当状态为已结束时，不能更改为进行中'
        //   });
        //   return false;
        // }
        // if (workThis.campaignStatus === 1303 && workThis.editCampaignStatus === 1302) {
        //   alert('kong')
        //   return false;
        // }
        ...
        console.log('入参的状态为：' + workThis.editCampaignStatus)
        workThis.$axios({
          method: 'post',
          url: '/ipos/campaign/update',
          data: {
            'header': {},
            'body': {
              ...
              'campaignStatus': workThis.editCampaignStatus,
          ...
          if (workThis.responseCode === '11122') {
            workThis.$message({
              type: 'warning',
              message: workThis.responseMess
            });
          }
          if (workThis.responseCode === '11123') {
            workThis.$message({
              type: 'warning',
              message: workThis.responseMess
            });
          }
          if (workThis.responseCode === '11124') {
            workThis.$message({
              type: 'warning',
              message: workThis.responseMess
            });
          }
          if (workThis.responseCode === '11125') {
            workThis.$message({
              type: 'warning',
              message: workThis.responseMess
            });
      ...
      selectAll() {
        ...
        workThis.$axios({
          method: 'post',
          url: '/ipos/campaign/selectCampaign',
          data: {
            'body': {
              'campaignId': workThis.$route.query.id
            }
          ...
          workThis.campaignStatus = workThis.campaign[0].campaignStatus;
          console.log('活动状态为：' + workThis.campaignStatus);
          console.log(workThis.campaign);
      ...
      lookTime() {
        ...
        workThis.campaignType = parseInt(workThis.$route.query.campaignType);
        workThis.image = workThis.$route.query.imagePath;
      ...
      changeMerchant() {
        ...
        workThis.$axios({
          method: 'post',
          url: '/ipos/campaign/selectMerchantName',
          data: {
            'body': {
              'campaignId': workThis.$route.query.id
            }
      ...
      changeCouponType() {
        ...
        if (workThis.sizeForm.inputCouponType === 2201) {
          workThis.$axios({
            method: 'post',
            url: '/ipos/campaign/getCouponName',
            data: {
              'body': {
                'couponType': 2201,
                'merchantsId': workThis.crmMerchantId
